package com.spldeolin.satisficing.app.common.lock;
...
import com.spldeolin.satisficing.app.common.lock.exception.LockNotAcquiredException;
...
public class StdLock {
     ...
     * @param key 分布式锁的key
     * @param waitSeconds 等待锁的最大秒数，小于等于0时代表不等待
     * @throws LockNotAcquiredException 没有获取到锁时抛出，如果不捕获，将会被统一异常处理当作"服务异常，请稍后重试"
     */
    public void dSync(Runnable criticalSection, String key, int waitSeconds) throws LockNotAcquiredException {
        log.info("StdLock: key={} waitSeconds={}", key, waitSeconds);
        RLock lock = redissonClient.getLock(key);
        boolean gotcha = false;

        try {
            gotcha = lock.tryLock(waitSeconds, TimeUnit.SECONDS);
